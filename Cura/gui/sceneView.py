...
from Cura.gui.util import openglGui  # 多次引用
...
class SceneView(openglGui.glGuiPanel):
	def __init__(self, parent):
		...
		# from Cura.util.printerConnection import printerConnectionManager   打印连接管理
		...
		# 引入openglGui,来自Cura\gui\util文件夹下
		# 需要剖析该处源码
		...
		self.rotateToolButton.setExpandArrow(True)    # setExpandArrow what is it ?
		self.scaleToolButton.setExpandArrow(True)     # setExpandArrow
		self.mirrorToolButton.setExpandArrow(True)    # setExpandArrow
		...
		# 调用切片引擎
		# from Cura.util import sliceEngine调用，第二个外部模块
		self._engine = sliceEngine.Engine(self._updateEngineProgress)

		# from Cura.gui.util import engineResultView调用，第三个外部模块
	...
	def loadFiles(self, filenames):
		...
		# only one GCODE file can be active  只有一个GCode文件能够活跃
		# so if single gcode file, process this    所以如果只有单一的GCode文件，处理这个
		# otherwise ignore all gcode files    而且，忽略其他所有的gcode文件
		gcodeFilename = None
		if len(filenames) == 1:
	...
	def reloadScene(self, e):
		...
		# 在DeleteAll清除之前，复制列表
		fileList = []
		pms_transforms = [] #position, rotation matrix, scale  位置，旋转 矩阵，大小规模
	...
	def updateToolButtons(self):
		if self._selectedObj is None:	
			...
			hidden = False
		self.rotateToolButton.setHidden(hidden)# 设置隐藏
	...
	def OnPaint(self,e):
		...
		if self.viewMode != 'gcode':
			...
			if self.viewMode == 'xray':
				...
				glDisable(GL_DEPTH_TEST)
				for i in xrange(2, 15, 2): #All even values  所有偶数的值
					...
					glVertex3f(-1000, 1000,-10)
					glEnd()
				for i in xrange(1, 15, 2): #All odd values  所有奇数的值
					glStencilFunc(GL_EQUAL, i, 0xFF)
		...
		if self.viewMode != 'gcode':
			...
			if self._selectedObj is not None:
				...
				for obj in self._scene.objects():
					...
					glEnd()
					glPopMatrix()
				if self._scene.isOneAtATime(): #Check print sequence mode.  证明打印的序列模型 
					glPushMatrix()
	...
	def _drawMachine(self):
		...
		if self._platformMesh[machine_type] is not None:
			...
			# 对于Ultimaker2 ,补偿纹理在底部的金属板，来展示Ultimaker2 文本
		...
		if self._platformTexture is None:
			self._platformTexture = openglHelpers.loadGLTexture('checkerboard.png')  # 棋盘格图片，底背景
	...
	def getObjectCenterPos(self):
		if self._selectedObj is None:
			return [0.0, 0.0, 0.0]
		pos = self._selectedObj.getPosition()  # 对 _selectedObj进行了多次调用 
		size = self._selectedObj.getSize()   # 对 _selectedObj进行了多次调用
		return [pos[0], pos[1], size[2]/2 - profile.getProfileSettingFloat('object_sink')]

	def getObjectBoundaryCircle(self):
		if self._selectedObj is None: # 对 _selectedObj进行了多次调用
			return 0.0
		return self._selectedObj.getBoundaryCircle() # 对 _selectedObj进行了多次调用

	def getObjectSize(self):
		if self._selectedObj is None:   # 对 _selectedObj进行了多次调用
			return [0.0, 0.0, 0.0]
		return self._selectedObj.getSize()    # 对 _selectedObj进行了多次调用

	def getObjectMatrix(self):
		if self._selectedObj is None:    # 对 _selectedObj进行了多次调用
			return numpy.matrix(numpy.identity(3))    
		return self._selectedObj.getMatrix()    # 对 _selectedObj进行了多次调用

#TODO: Remove this or put it in a seperate file
# todo:移除这个 将 它放在独立的文件中
