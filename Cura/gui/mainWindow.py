...
##################################以上5个菜单，全部结束###########################
		...
		# BoxSizer 
		# Google翻译一个框分级机背后的基本想法是，窗口将最多以相当简单的基本几何形状布置，通常在一排或一列或任几个层次结构。
		...
		# 从参数选择文件中，还原窗口位置，大小，状态，
		try:
			if profile.getPreference('window_maximized') == 'True':    # 最大
				self.Maximize(True)
			else:
				...
				if posx > 0 or posy > 0:
					self.SetPosition((posx,posy))    # 设置x,y坐标
				if width > 0 and height > 0:
					self.SetSize((width,height))    # 设置宽，高

			self.normalSashPos = int(profile.getPreference('window_normal_sash'))    # 设置正常
		except:
	...
	def onPluginUpdate(self,msg): #receives commands from the plugin thread 从插件的进程中获得命令
	...
	#################################################方法较多,还没抄写完3月4日写#######################################
	#################################################方法较多,3月5日转换工作方向，仍旧无精力对一下代码进行分析#######################################
	#################################################方法较多,3月6日稍微看一下#######################################
	#################################################方法较多,3月8日,今天状态不佳#######################################
...
class normalSettingsPanel(configBase.configPanelBase):
	...
	def __init__(self, parent, callback = None):
		...
		(left, right, self.printPanel) = self.CreateDynamicConfigTab(self.nb, _('Basic'))    # 右边的参数Basic
		self._addSettingsToPanels('basic', left, right)
		self.SizeLabelWidths(left, right)

		(left, right, self.advancedPanel) = self.CreateDynamicConfigTab(self.nb, _('Advanced'))    # 右边
